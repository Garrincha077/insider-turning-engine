...
class SECIncrementalSource:
    ...
    def _response_body(response: Any) -> bytes:
        ...
        if callable(iterator):
            return b"".join(bytes(chunk) for chunk in iterator())
        raise ValueError("SEC response did not contain a body")

    def _request(self, url: str, accept: str) -> tuple[bytes, Mapping[str, str], int]:
        ...
        for attempt in range(self.max_attempts):
            ...
            try:
                ...
                return self._response_body(response), response_headers, status
    ...
    def _read_submissions(self, cik: str) -> tuple[dict[str, Any] | None, list[QuarantineRecord]]:
        ...
        try:
            body, _, _ = self._request(url, "application/json")
        except SECRequestError as exc:
            self._available = False
                ...
                json.dumps(
                    {
                        ...
                        "retrieved_at": utc(self.clock()).isoformat(),
