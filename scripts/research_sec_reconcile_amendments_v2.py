...
Version 2 keeps the SEC original-submission accession as research linkage evidence
but drives multi-amendment chains through the immediate ``supersedesRevisionId``.
That distinction prevents a later amendment from falsely declaring the root
accession as its immediate predecessor.
"""
...
def reconcile(*, original_root: Path, amendment_root: Path, catalog_root: Path, output: Path) -> dict[str, Any]:
    ...
    all_input = [*originals, *amendments]
    if any(record.timestamps.knowledge_at.year >= 2023 for record in all_input):
        raise ValueError("sealed OOS boundary violated by 2023+ canonical evidence")
    if any(record.timestamps.knowledge_at != record.timestamps.accepted_at for record in all_input):
    ...
    for accession in accession_order:
        ...
        if status == "LINKED_TO_ORIGINAL_BUY_UNIVERSE" and root_accession is not None:
            ...
            for amendment in rows:
                ...
                if root is None or predecessor_revision is None:
                    ...
                    continue
                # The root accession remains in amendment-linkage.jsonl as evidence.
                # The production resolver is instead given the immediate revision ID;
                # leaving a root accession on a second amendment would conflict with
                # that immediate predecessor by design.
                source = amendment.source.model_copy(update={"amends_accession_number": None})
        ...
        elif status != "PREDECESSOR_OUTSIDE_ORIGINAL_BUY_UNIVERSE":
            for amendment in rows:
                research_quarantines.append(
                    {
                        "accession": accession,
    ...
    resolution_input = [*originals, *linked_amendments]
    point_in_time = max(record.timestamps.knowledge_at for record in resolution_input) + timedelta(seconds=1)
    ...
    _write_jsonl(output / "quarantines.jsonl", [*research_quarantines, *resolver_quarantines])
    ...
    summary = {
        ...
        "linkagePolicy": {
            ...
            "chainEvidence": "immediate supersedesRevisionId after root linkage",
