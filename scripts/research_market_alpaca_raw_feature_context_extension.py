"""Outcome-blind Alpaca SIP raw feature context extension for 2021-2022."""
...
def _load_context_symbols(
    source: Path,
    year: int,
) -> tuple[list[str], dict[str, object]]:
    if not MIN_YEAR <= year <= MAX_YEAR:
        raise ValueError("raw feature-context extension year must be 2021-2022")

    wanted_years = {year}
    if year < MAX_YEAR:
        wanted_years.add(year + 1)
    ...
    if not symbols:
        raise ValueError(f"no symbols found for raw context extension year {year}")
...
def run(*, source: Path, year: int, output: Path) -> dict[str, object]:
    ...
    headers = {
        ...
        "User-Agent": "InsiderTurningEngine-Research-FeatureContextExtension/1.0",
    }
    ...
    summary: dict[str, object] = {
        "schemaVersion": "1.0.0",
        "dataset": "Alpaca SIP raw feature-formation market context extension",
        "year": year,
        "feed": "sip",
        "adjustment": "raw",
        ...
        **diagnostics,
        ...
        "status": (
            "ALPACA_RAW_FEATURE_CONTEXT_EXTENSION_PASS"
            if not failures and total_rows
            else "ALPACA_RAW_FEATURE_CONTEXT_EXTENSION_FAIL"
        ),
    }
    ...
    if failures:
        raise RuntimeError(
            f"raw feature context extension failed with {len(failures)} failures"
        )
    return summary
