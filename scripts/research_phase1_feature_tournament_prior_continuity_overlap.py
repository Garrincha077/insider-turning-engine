"""Measure safe reuse of frozen B1/B3 continuity evidence for the feature tournament.

No performance or market-price field is read. This stage only matches already
frozen continuity classifications onto the exact unresolved Stage-B semantic
event-horizon keys.
"""
...
def _fingerprint(row: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "resultState": str(row.get("resultState") or ""),
        ...
        "quantityFactor": "" if quantity in (None, "") else str(quantity),
        "cashPerEntryShare": (
            "" if row.get("cashPerEntryShare") in (None, "") else str(row.get("cashPerEntryShare"))
        ),
...
def _compatible(rows: list[dict[str, Any]]) -> bool:
    if len(rows) <= 1:
        return True
    first = _fingerprint(rows[0])
    return all(_fingerprint(row) == first for row in rows[1:])
...
def run(
    *,
    ...
    match_sources_combo: Counter[str] = Counter()
    safe_rows = 0
    conflict_rows = 0
    unmatched_rows = 0
    ...
    for row in scope_rows:
        ...
        combo = "+".join(sources) if sources else "NONE"
        match_sources_combo[combo] += 1

        compatible = bool(matches) and _compatible(matches)
        if compatible:
            safe_rows += 1
            status = "SAFE_PRIOR_EVIDENCE_MATCH"
        elif matches:
            conflict_rows += 1
            status = "PRIOR_EVIDENCE_CONFLICT"
        else:
            unmatched_rows += 1
        ...
        results.append(
            {
                **_key_object(row),
                ...
                "currentMaxInternalGapSessions": int(
    ...
    safe_match_rows = [
        row for row in results if row["matchStatus"] == "SAFE_PRIOR_EVIDENCE_MATCH"
    ]
    payload = {
        "schemaVersion": "1.0.0",
        "status": "PHASE1_FEATURE_TOURNAMENT_PRIOR_CONTINUITY_OVERLAP_COMPLETE",
        "researchOnly": True,
        "performanceRead": False,
        "priceFieldsRead": [],
        "featureOutcomesRead": False,
        ...
        "priorContracts": {
            "B1": {
                "contractId": b1.get("contractId"),
                "rows": len(b1_rows),
            },
            "B3": {
                "status": b3.get("status"),
                "rows": len(b3_rows),
                "classifiedKeySha256": b3.get("classifiedKeySha256"),
            },
        },
        "sourceHitRows": dict(sorted(source_hits.items())),
        "sourceCombinationRows": dict(sorted(match_sources_combo.items())),
