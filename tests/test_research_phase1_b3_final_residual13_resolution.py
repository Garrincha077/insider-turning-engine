...
def _contract() -> dict[str, object]:
    ...
    for (cik,ticker), rows in mod.EXPECTED_COUNTS.items():
        ...
        if ticker=="ARWA":
            fact.update(mode="COMMON_TO_SUCCESSOR_COMMON", effectiveDate="2016-12-28",
                        successorSymbol="VVPR", successorIssuerCik="0001681348")
        elif ticker=="ATACU":
            fact.update(mode="UNIT_TO_COMMON_PLUS_RIGHT", effectiveDate="2018-08-22",
                        successorSymbol="HFFG", successorSharesPerEntryShare=1.1)
        elif ticker=="TPGE":
            fact.update(mode="COMMON_SYMBOL_CHANGE", effectiveDate="2018-08-01",
                        successorSymbol="MGY")
        elif ticker=="NBA.U":
            fact={
                "issuerCik":cik,"ticker":ticker,"expectedRows":rows,
    ...
    return {
        ...
        "oosOpened":False,"productionScoringChanged":False,
        "identities":facts,
    }
...
def test_nba_basket_cannot_drop_warrant(tmp_path: Path) -> None:
    p=_contract()
    nba=next(x for x in p["identities"] if x["ticker"]=="NBA.U")
...
def test_arwa_post_close_is_one_vvpr_not_unit_bundle() -> None:
    row={"eventNumber":1,"issuerCik":"0001622577","ticker":"ARWA",
         "evaluationSession":"2016-06-30","entrySession":"2016-07-01",
         "horizon":126,"targetExitSession":"2016-12-30","pivotDate":"2016-07-22"}
    fact=next(x for x in _contract()["identities"] if x["ticker"]=="ARWA")
...
def test_atacu_pre_close_target_remains_unit() -> None:
    row={"eventNumber":1,"issuerCik":"0001680873","ticker":"ATACU",
         "evaluationSession":"2017-08-25","entrySession":"2017-08-28",
         "horizon":126,"targetExitSession":"2018-02-28","pivotDate":"2018-01-16"}
    fact=next(x for x in _contract()["identities"] if x["ticker"]=="ATACU")
    out=mod._resolve_row(row,fact)
