...
const candidateColumns = candidateColumn.columns([
  ...
  candidateColumn.accessor('marketRs', { header: 'MRS Mkt', cell: (info) => <span className={info.getValue() >= 0 ? 'font-mono text-xs text-emerald-300' : 'font-mono text-xs text-rose-300'}>{signed(info.getValue())}</span> }),
  candidateColumn.accessor((row) => (row.currentPrice / row.insiderCost - 1) * 100, { id: 'costPl', header: 'Cost P/L', cell: (info) => <span className="font-mono text-xs text-amber-200">{signed(info.getValue())}%</span> }),
...
function RadarView({ data, selected, onSelect, onOpenLab }: { data: DashboardData; selected?: Candidate; onSelect: (ticker: string) => void; onOpenLab: () => void }) {
  return <>
    ...
    <div className="grid gap-6 2xl:grid-cols-[minmax(0,1fr)_340px]">
      ...
      {selected && <ReasonPanel candidate={selected} onOpen={onOpenLab} />}
    </div>
  </>;
}
...
function PulseCards({ data }: { data: DashboardData }) {
  const metrics = [
    { label: 'Transaction', value: '1.42', move: '+0.18' },
    { label: 'Unique insiders', value: '1.67', move: '+0.24' },
    { label: 'Dollar', value: '0.71', move: '-0.06' },
    { label: 'Breadth', value: '58.3%', move: '+4.1' },
  ];
  return <div className="grid gap-4 lg:grid-cols-[1.3fr_repeat(4,1fr)]">
    <div className="relative overflow-hidden rounded-xl border border-emerald-400/20 bg-[linear-gradient(135deg,rgba(20,184,166,.17),rgba(15,23,42,.2))] p-5">
      ...
      <div className="mt-4 flex items-end gap-3"><span className="font-mono text-5xl font-semibold tracking-[-0.06em]">{data.marketPulse}</span><span className="mb-1.5 text-xs font-semibold text-emerald-300">↗ 8.4 / 4W</span></div>
      <div className="mt-5 h-1.5 overflow-hidden rounded-full bg-slate-950/50"><div className="h-full rounded-full bg-gradient-to-r from-sky-400 via-emerald-400 to-lime-300" style={{ width: `${data.marketPulse}%` }} /></div><p className="mt-2 text-xs text-slate-400">{data.pulsePercentile}th historical percentile · bullish accumulation</p>
    </div>
    {metrics.map((metric) => <div key={metric.label} className="rounded-xl border border-border bg-card p-4"><p className="text-[10px] font-semibold uppercase tracking-[0.13em] text-muted-foreground">{metric.label}</p><p className="mt-5 font-mono text-2xl font-semibold">{metric.value}</p><p className={`mt-2 text-xs ${metric.move.startsWith('+') ? 'text-emerald-300' : 'text-rose-300'}`}>{metric.move} vs 4W</p></div>)}
  </div>;
}
...
function CandidateList({ data, mode, selectedTicker, onSelect }: { data: DashboardData; mode: 'turning' | 'divergence'; selectedTicker: string; onSelect: (ticker: string) => void }) {
  ...
  return <div className="grid gap-6 2xl:grid-cols-[minmax(0,1fr)_340px]"><CandidateTable candidates={sorted} selectedTicker={selectedTicker} onSelect={onSelect} /><ReasonPanel candidate={sorted.find((candidate) => candidate.ticker === selectedTicker) ?? sorted[0]} onOpen={() => undefined} /></div>;
}

function SmartBuysView({ data }: { data: DashboardData }) {
  return <Panel title="Highest-conviction purchases" subtitle="Qualified open-market buys only"><div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{data.filings.filter((filing) => filing.side === 'BUY').map((filing) => <div key={filing.accession} className="rounded-lg border border-border bg-background/35 p-4"><div className="flex items-center justify-between"><span className="font-mono text-lg font-semibold text-emerald-200">{filing.ticker}</span><Badge className="bg-emerald-400/10 text-emerald-200">BUY</Badge></div><p className="mt-4 text-sm font-medium">{filing.owner}</p><p className="text-xs text-muted-foreground">{filing.role}</p><p className="mt-4 font-mono text-2xl font-semibold">${compactMoney(filing.value)}</p><p className="mt-1 text-[10px] text-muted-foreground">Filed {filing.filedAt} UTC</p></div>)}</div></Panel>;
}
...
function BacktestLab({ data }: { data: DashboardData }) {
  ...
  return <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_330px]"><Panel title="Forward excess returns vs SPY" subtitle="2016–present · illustrative sample until pipeline export"><EChart option={option} style={{ height: 410 }} /></Panel><Panel title="Validation gate" subtitle="Sealed OOS: 2023–latest"><div className="space-y-4 text-xs text-slate-300"><Metric label="OOS events" value="— / 200" /><Metric label="Primary horizon" value="6M" /><Metric label="Current outcome" value="INCONCLUSIVE" accent /><p className="rounded-lg border border-amber-300/20 bg-amber-300/8 p-3 leading-5 text-amber-100">Telegram remains off until the point-in-time backtest meets the formal PASS criteria.</p></div></Panel></div>;
}
